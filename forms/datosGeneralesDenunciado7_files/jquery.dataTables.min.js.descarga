...
this.aanFeatures = []; this.oLanguage = { sProcessing: "Processing...", sLengthMenu: "Ver _MENU_ registros", sZeroRecords: "La busqueda no arrojo resultados", sEmptyTable: "No data available in table", sInfo: "Viendo _START_ a _END_ de _TOTAL_ registros", sInfoEmpty: "Showing 0 to 0 de 0 registros", sInfoFiltered: "(filtered from _MAX_ registros)", sInfoPostFix: "", sSearch: "Search:", sUrl: "", oPaginate: { sFirst: "Primero", sPrevious: "Anterior", sNext: "Siguiente", sLast: "Ultimo" }, fnInfoCallback: null }; this.aoData = []; this.aiDisplay = []; this.aiDisplayMaster =
...
this.fnFormatNumber=function(a){if(a<1E3)return a;else{var b=a+"";a=b.split("");var c="";b=b.length;for(var d=0;d<b;d++){if(d%3===0&&d!==0)c=","+c;c=a[b-d-1]+c}}return c};this.aLengthMenu=[5,10,25,50,100];this.bDrawing=this.iDraw=0;this.iDrawError=-1;this._iDisplayLength=5;this._iDisplayStart=0;this._iDisplayEnd=10;this._iRecordsDisplay=this._iRecordsTotal=0;this.bJUI=false;this.oClasses=n.oStdClasses;this.bSorted=this.bFiltered=false;this.oInit=null}function r(a){return function(){var b=[A(this[n.iApiIndex])].concat(Array.prototype.slice.call(arguments));
...
c=b.getElementsByTagName("table")[0];b.style.width=v(a.nTable.offsetWidth+a.oScroll.iBarWidth);c.style.width=v(a.nTable.offsetWidth)}if(a.bSorted||a.bFiltered)d.scrollTop=0}function X(a){if(a.oFeatures.bAutoWidth===false)return false;$(a);for(var b=0,c=a.aoColumns.length;b<c;b++)a.aoColumns[b].nTh.style.width=a.aoColumns[b].sWidth}function xa(a){var b=p.createElement("div");a.sTableId!==""&&typeof a.aanFeatures.f=="undefined"&&b.setAttribute("id",a.sTableId+"_filter");b.className=a.oClasses.sFilter;
    b.innerHTML = a.oLanguage.sSearch + (a.oLanguage.sSearch === "" ? "" : " ") + '<input type="text" placeholder="Digite Busqueda..." />'; var c = j("input", b); c.val(a.oPreviousSearch.sSearch.replace('"', "&quot;")); c.bind("keyup.DT", function () { for (var d = a.aanFeatures.f, f = 0, e = d.length; f < e; f++) d[f] != this.parentNode && j("input", d[f]).val(this.value); this.value != a.oPreviousSearch.sSearch && P(a, { sSearch: this.value, bRegex: a.oPreviousSearch.bRegex, bSmart: a.oPreviousSearch.bSmart }) }); c.bind("keypress.DT", function (d) { if (d.keyCode == 13) return false }); return b
}
